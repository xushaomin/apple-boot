...
SERVER_CONTAINER=`sed '/application.container/!d;s/.*=//' conf/system.properties | tr -d '\r'`
...
nohup $JAVA_HOME/bin/java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.appleframework.boot.Main env=$ENV container=$SERVER_CONTAINER > $STDOUT_FILE 2>&1 &
